...
`prepare-solver` は、正典、ケース（`ask`/`key` 双方）、suite 設定、指示テンプレート（`solver.md`・
`judge.md`）の SHA-256 を control manifest に固定する。以後どれかが変更されると、回答封印・judge
bundle 生成・採点は停止する。候補回答を見てから `key`（正解側）を書き換えることはできない。
テンプレートを含めるのは、`judge.md` が採点の判定規則を持つためである。ここを書き換えると、同じ
ケース・同じ正典でも結果が変わり、版間の比較が成り立たなくなる。変更したら新しい run を起こす。
