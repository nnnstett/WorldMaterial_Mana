...
公理から導出される世界の法則（[T1 エネルギー保存則]〜[T6 同調], [T10 集合無意識]）と、その行使のかたちへの命名である**行使形態の定義**（[T7 魔法]〜[T9 超能力]）。一部の定理は、強く結びつく短い帰結を本文中の**節**として持つ（独立 ID は付かない）。「概要」は専門語を避けた一行解説で、正確な本文は定理名のリンク先を参照。
