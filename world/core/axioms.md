...
**公理**（[凡例](../README.md#役割ラベルの凡例)）:
1. 共鳴（→ [I2 共鳴則]）を経た現化の生成物は、駆動した個体が知覚している場（現に感じ取っている場）に現れる。知覚が確かな場ほど精密に、不確かな場ほど大まかにしか現れず、全く感じ取れていない場には現れない <a id="i6-出現位置"></a>
2. マナから物質への変換は、その場を一挙に置き換えるのではなく、形を成していく過程（形成）として進む。生成物は既存の物質と同じ場所を占められず、変換は、既存の物質の押しのけ・圧縮に必要なエネルギーを変換の一部として伴う <a id="i6-形成過程"></a>
...
**補足**:
- 変換先（何になるか。→ [I2 共鳴則]）には、物質・エネルギーの種類だけでなく、温度や相（固体・液体・気体の別）、初期の運動までの状態が含まれる。初期の運動は駆動した個体を基準に定まり、指定がなければ静止して現れる
