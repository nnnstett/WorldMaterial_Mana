...
2. マナから物質への変換は、その場を一挙に置き換えるのではなく、形を成していく過程（形成）として進む。生成物は既存の物質と同じ場所を占められず、変換は、既存の物質の押しのけ・圧縮に必要なエネルギーを伴う <a id="i1-形成過程"></a>
...
**補足**:
- 変換先（何になるか）には、物質・エネルギーの種類だけでなく、温度や相（固体・液体・気体の別）、初期の運動までの状態が含まれる。初期の運動は駆動した個体を基準に定まり、指定がなければ静止して現れる
